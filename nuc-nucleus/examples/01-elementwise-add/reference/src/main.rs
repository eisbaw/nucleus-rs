//! Reference implementation of example 01-elementwise-add.
//!
//! `c[i] = a[i] + b[i]` over an array of N i32 little-endian words.
//!
...
//!   - `i32::wrapping_add` makes overflow behaviour explicit; this
//!     example's fixtures are within range and never overflow in
//!     practice, but the choice documents intent.
...
fn main() -> ExitCode {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "-h" | "--help" => {
                eprintln!(
                    "usage: elementwise-add-reference --in INPUT.bin --out OUTPUT.bin"
                );
...
fn die(msg: &str) -> ExitCode {
    eprintln!("elementwise-add-reference: {}", msg);
    ExitCode::FAILURE
}
