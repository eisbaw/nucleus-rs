//! Reference implementation of example 08-histogram.
//!
//! Single-pass integer histogram: for each input value `v` in
//! [0, BINS-1], increment `histogram[v]` by one. Output is a
//! BINS-wide i32 LE array. The naive direct-indexing loop here is
//! the equivalent of what the algorithm's rectangular masked
//! accumulator computes — but the reference is structurally
//! different (direct index vs. mask-and-compare), so a backend
//! whose output matches the reference bit-for-bit is unlikely to
//! be "wrong in the same way" as the reference
//! (docs/reference-impl-policy.md §2).
//!
//! Independence from kernels.rs is deliberate: kernels.rs does
//! `if value == bin { acc + 1 } else { acc }` per (i, b) cell of
//! the rectangular nest. This reference does
//! `hist[value] += 1` per i — different access pattern, same
//! result. The output bit-identity is what validates that the
//! masked-accumulator shape lowers correctly.
//!
//! Input format (`input.bin`):
//!   - bytes [0 .. 4*N) — N i32 little-endian words. Each value
//!     must be in [0, BINS-1]; the harness validates this strictly
//!     (silent acceptance of out-of-range input would mask a
//!     fixture-drift bug).
//!
//! Output format (`reference.bin`):
//!   - bytes [0 .. 4*BINS) — BINS i32 LE words, the histogram.
//!
//! N, BINS are fixed at the values declared in `prog.algo.nuc`. If
...
fn main() -> ExitCode {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "-h" | "--help" => {
                eprintln!("usage: histogram-reference --in INPUT.bin --out OUTPUT.bin");
    ...
    // Decode and validate each value is in [0, BINS-1]. Out-of-
    // range values would silently miss every bin in the
    // rectangular nest (kernels.rs::bin_inc returns acc when no
    // bin matches) — the reference rejects them loudly so the
    // fixture drift surfaces here, not as a silent zero-count.
    ...
    // Single-pass histogram. Strict left-to-right; `wrapping_add`
    // documents overflow intent. The committed fixture's max bin
...
fn die(msg: &str) -> ExitCode {
    eprintln!("histogram-reference: {}", msg);
    ExitCode::FAILURE
}
