...
//!
//! # Why this module lives in `nucleus-compiler`
//!
...
//!
//! Moving NameTables into nucleus-compiler dissolves both. test-common can
...
/// `nucleus-compiler/tests/petri_to_events.rs` perform. The driver
/// builds these from the post-pass ACFG; the backend never sees the
/// ACFG itself.
///
/// Pre-TASK-0238 history: this struct lived in `pthreads-sync`. Moved
/// to `nucleus-compiler` so the cross-backend test-helper crate
/// `test-common` (TASK-0237) can return a pre-built instance instead
/// of 5 raw BTreeMaps + a BTreeSet.
