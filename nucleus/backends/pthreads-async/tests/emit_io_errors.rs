...
//! `EmitError` variants on the pthreads-async backend.
//!
...
//! pthreads-async so a future refactor that re-introduces an
...
//! (the #1 recurring "we thought we fixed this" defect class).
//!
//! Each failure mode is induced deterministically and portably (no
//! permission-bit reliance, which is unreliable as root / in CI):
//! a missing read path, a path component that is a regular file, and a
//! write target that is a directory.
//!
...
//!
//! pthreads-async first generated write (the `WriteFailed` target) is
//! `out_dir/Cargo.toml` (shared `render_cargo_toml`); its
...
use pthreads_async::{emit, EmitError, NameTables};

/// Locate the repo root from this crate's manifest (mirrors
/// `tests/skeleton.rs`'s `repo_root`).
...
fn scratch_dir(name: &str) -> PathBuf {
    let target = repo_root().join("nucleus/target/pthreads-async-test-scratch");
    test_common::unique_scratch_dir(&target, name)
}

/// Lower example 01-elementwise-add / naive (single-worker) into the
/// `(per_worker, names, sidecar, kernels.rs path)` contract inputs every
/// backend `emit` consumes. Single-worker reaches the same I/O sites as
/// multi-worker but never trips a multi-worker ContractGap, so the I/O
/// failure is the FIRST thing that can fail (real teeth).
...
/// `KernelsReadFailed`: the first I/O op in `emit` is
/// `fs::read_to_string(kernels_rs_path)`; a missing path fails it before
/// any rendering or output creation.
#[test]
...
/// then writes `out_dir/Cargo.toml` — the FIRST generated write for
/// pthreads-async. Pre-creating that path as a DIRECTORY makes
/// `fs::write` fail.
#[test]
        ...
        EmitError::WriteFailed { path, .. } => {
            assert_eq!(
                ...
                "WriteFailed must name the Cargo.toml path it could not write \
                 (pthreads-async's first generated write)"
            );
        }
