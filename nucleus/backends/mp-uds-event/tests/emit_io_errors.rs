...
//! `EmitError` variants on the mp-uds-event backend.
//!
...
//! mp-uds-event so a future refactor that re-introduces an
...
//!
//! mp-uds-event layout difference vs pthreads-sync: the first generated
...
use mp_uds_event::{emit, EmitError, NameTables};
...
fn scratch_dir(name: &str) -> PathBuf {
    let target = repo_root().join("nucleus/target/mp-uds-event-test-scratch");
...
/// `WriteFailed`: pre-creating `out_dir/src/kernels.rs` (mp-uds-event's
        ...
        EmitError::WriteFailed { path, .. } => {
            assert_eq!(
                ...
                "WriteFailed must name the src/kernels.rs path it could not write \
                 (mp-uds-event's first generated write)"
            );
        }
