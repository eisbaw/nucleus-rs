...
// --------------------------------------------------------------------
//
// `render_cargo_toml` / `render_run_sh` are `pub` so the pthreads-async
// backend's single-worker arm (TASK-0226) can reuse them VERBATIM and
...
/// Emit the run.sh the generated project ships. Public so
/// pthreads-async (TASK-0226 single-worker arm) reuses it.
