...
//! `EmitError` variants on the mp-tcp-bufsync backend.
//!
...
//! mp-tcp-bufsync so a future refactor that re-introduces an
...
//!
//! mp-tcp-bufsync layout difference vs pthreads-sync: the first generated
//! write (the `WriteFailed` target) is `out_dir/src/kernels.rs` — it is
//! written BEFORE the single-vs-multi-worker branch, ahead of Cargo.toml.
//! Its `OutputCreateFailed` directory is `out_dir/src/bin`.

use std::collections::BTreeMap;
use std::fs;
use std::path::{Path, PathBuf};

use mp_tcp_bufsync::{emit, EmitError, NameTables};
...
fn scratch_dir(name: &str) -> PathBuf {
    let target = repo_root().join("nucleus/target/mp-tcp-bufsync-test-scratch");
...
/// `create_dir_all(out_dir/src/bin)` fail.
#[test]
...
/// `WriteFailed`: pre-creating `out_dir/src/kernels.rs` (mp-tcp-bufsync's
...
fn write_failed_when_first_write_target_is_a_directory() {
    ...
    // The first write is `out_dir/src/kernels.rs`; `out_dir/src` is
    // created by `create_dir_all(out_dir/src/bin)` just before, so we
    // can pre-create the kernels.rs target as a directory.
        ...
        EmitError::WriteFailed { path, .. } => {
            assert_eq!(
                ...
                "WriteFailed must name the src/kernels.rs path it could not write \
                 (mp-tcp-bufsync's first generated write)"
            );
        }
