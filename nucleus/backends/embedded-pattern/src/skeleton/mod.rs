...
pub trait NucleusShim {
    ...
    /// IRQ-driven completion wait for a `notify=event` DMA transfer on
    /// channel `seq` (TASK-0455.02). This is the `notify`-selected
    /// completion path the double-buffered (`buffer=2`) `mode=dma` ring
    /// emits when the schedule asks for `notify=event`, vs the
    /// `while !dma_link_poll {}` busy-spin for the default/poll modes. On a
    /// real async DMA engine this would `wfi` until the DMA-complete IRQ
    /// fires (the CPU doing useful work meanwhile); the DEFAULT impl
    /// delegates to `dma_link_poll` in a spin because the MODELLED shim
    /// completes the transfer synchronously inside the arm call (no real
    /// IRQ fires under Renode — a `wfi` would block forever). Being
    /// defaulted, NO shim (`StubShim`, `MultiMcuShim`, `Usart1Shim`,
    /// `NrfUarteShim`) overrides it; a `notify=event` DMA edge therefore
    /// moves bytes correctly over the existing transport with zero new
    /// per-shim impl code. The structural distinction (an event-wait method
    /// call, not a bare poll-spin) IS the honoured `notify=event` semantics
    /// the schedule requested (the first consumer of `XferFacts::notify`).
    fn dma_link_irq_wait(&mut self, seq: usize) {
        while !self.dma_link_poll(seq) {
            core::hint::spin_loop();
        }
    }
...
mod tests {
    ...
    fn render_lib_emits_no_std_and_run() {
        ...
        // No buffered DMA edge → no ring statics either.
        let s = render_lib(
