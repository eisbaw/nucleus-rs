...
//! `EmitError` variants on the mp-tcp-event backend.
//!
...
//! mp-tcp-event so a future refactor that re-introduces an
...
//!
//! mp-tcp-event layout difference vs pthreads-sync: the first generated
...
use mp_tcp_event::{emit, EmitError, NameTables};
...
fn scratch_dir(name: &str) -> PathBuf {
    let target = repo_root().join("nucleus/target/mp-tcp-event-test-scratch");
...
/// `WriteFailed`: pre-creating `out_dir/src/kernels.rs` (mp-tcp-event's
        ...
        EmitError::WriteFailed { path, .. } => {
            assert_eq!(
                ...
                "WriteFailed must name the src/kernels.rs path it could not write \
                 (mp-tcp-event's first generated write)"
            );
        }
