//! Multi-worker SPMD codegen for the `mpi-nonblocking` backend
//! (TASK-0046, M8). Lowers the per-worker [`Event`] lists of a ≥2-worker
...
//! memory — cross-worker data crosses the network via MPI, here a
//! NON-BLOCKING BUFFERED send (`MPI_Ibsend`) + non-blocking receive
//! (`MPI_Imrecv` / `MPI_Irecv`) with an explicit `MPI_Wait`.
//!
...
//! — the same walker pthreads-sync / pthreads-async / mp-tcp-event /
//! mpi-blocking use. This backend emits a per-rank PRELUDE of MPI
//! rendezvous + barrier wrapper types whose `.push()` / `.wait()` methods
//! the walker targets (`rendezvous_prefix = "mpi"`), so the Fire/Loop
//! codegen cannot drift from the in-process backends OR from mpi-blocking.
//! The Plan-building logic (host election, rank assignment, channel-id
//! collection, barrier participant analysis, the two loud rejects) is
...
//!   and **MPI tag = `<rid>`**, lowering to a buffered `MPI_Ibsend` that
//!   completes LOCALLY (the payload is copied into the process-attached
//!   send buffer), so the push never blocks on the matching receive. The
//!   per-rid tag is LOAD-BEARING: without it, two messages with the same
//!   `(src, dst)` would match in MPI send-order, so a consumer that posts
//!   its receives in a different order than the producer sent would
//!   silently swap payloads — the exact value-bug a single `mpiexec -n 1`
//!   run hides and an all-ranks-live `-n N` run exposes (memory
//!   `16-jacobi`: deadlock-free != value-correct).
//! - `Wait{data, src, seq}` -> `let <data> = mpi_<rid>.wait();` (whole-
//!   array, `MPI_Mprobe` + `MPI_Imrecv` + `MPI_Wait`) or the gather
//!   slice-paste the shared walker emits, with the same tag = `<rid>` so
//!   it matches the producer's `Push`.
...
//! model permits. Under blocking `MPI_Send` (mpi-blocking) a message above
//! the eager limit blocks until its receive is posted, so that ordering
//! can DEADLOCK. Standard-mode `MPI_Isend` would avoid the block but its
//! request borrows the send buffer until `Wait`, forcing the `Wait` to be
//! deferred past the matching receive for any overlap — which the linear
//! `.push(v)` walker API cannot express without a self-referential
//! buffer/request or unbounded leaking. Buffered `MPI_Ibsend` sidesteps
//! both: it copies the payload into the attached buffer and completes
//! locally, so `push` can `Wait` immediately (clean buffer lifetime) yet
//! never blocks on the receiver. The cost is the attached buffer (see
//! [`Plan::bsend_bytes`]); the benefit is genuine deadlock-immunity.
//!
...
//!   (the Comm_split work item, shared with mpi-blocking). This is why
//!   09-producer-consumer/pipelined (barrier `{producer,consumer}`
//!   excluding host) is NOT yet a runnable target (TASK-0046 AC#3).
...
//!   TASK-0045.03. No shipped multi-worker async schedule carries a check
...
impl<'a> Plan<'a> {
    fn build(
        ...
        let host_worker =
            elect_host_from_worker_names(&names.worker, &used_workers).ok_or_else(|| {
                EmitError::ContractGap(
                    "multi-worker mpi-nonblocking emit requires at least one used worker".to_string(),
                )
            })?;
        ...
        for w in &used_workers {
            if has_check_frame(&per_worker[w]) {
                return Err(EmitError::UnsupportedFeature(
                    "mpi-nonblocking: a `check loop` frame on a multi-worker schedule is not yet \
        ...
        // invariant ever regressed, `check-mpi-nonblocking`'s `timeout`
        // wrapper would catch the resulting deadlock loud.
        ...
        for (tag, parts) in &barrier_participants {
            if *parts != all_used {
                ...
                return Err(EmitError::UnsupportedFeature(format!(
                    "mpi-nonblocking: barrier (SyncTag {}) has a non-whole-world participant set \
    ...
    fn emit(&self) -> Result<String, EmitError> {
        ...
        writeln!(
            out,
            "//! Generated by the nucleus pre-compiler (mpi-nonblocking backend, SPMD multi-worker)."
        )
        .ok();
        ...
        .ok();
        // ---- Buffered-send buffer attach (MPI_Buffer_attach). ----
        // Buffered `Ibsend` copies each payload here and completes
        // locally, so a Push never blocks on the matching Wait (the
        // deadlock fix). Sized from the schedule's data footprint with
        // headroom; `NUC_MPI_BSEND_BYTES` overrides for an unusually large
        // in-flight set. A too-small buffer fails LOUD (MPI_ERR_BUFFER),
        // never silent corruption. Attached BEFORE `world` is taken (the
        // attach needs `&mut universe`).
        let bsend = self.bsend_bytes();
        writeln!(
            out,
        ...
        writeln!(
            out,
            "            eprintln!(\"mpi-nonblocking: this schedule needs exactly {n} ranks (got {{size}}); launch `mpiexec -n {n}`\");"
        )
        .ok();
        writeln!(out, "        }}").ok();
...
";
    ...
    /// (`MPI_Buffer_attach`, emitted in `main` via
    /// `Universe::set_buffer_size`). Buffered `Ibsend` copies each payload
    /// here, so the buffer must hold every simultaneously-outstanding
