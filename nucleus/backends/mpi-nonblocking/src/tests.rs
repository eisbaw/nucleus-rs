//! Single-worker SPMD emit pins + the multi-worker buffered-rendezvous
//! shape / forward-link guards for mpi-nonblocking (TASK-0046, M8).
//! In-crate (`#[cfg(test)] mod tests;`) so it can pin the private
//! compute-body delegation against the shared renderer directly.
...
fn repo_root() -> PathBuf {
    // .../nucleus/backends/mpi-nonblocking -> backends -> nucleus -> repo.
...
fn single_worker_emit_shape_and_compute_delegation() {
    ...
    let scratch = repo_root().join("nucleus/target/mpi-nonblocking-test-scratch/single_worker_01");
    let _ = std::fs::remove_dir_all(&scratch);

    let res = emit(&r.per_worker, &r.names, &r.sidecar, &kernels, &scratch)
        .expect("mpi-nonblocking emit (single-worker 01-elementwise-add)");
...
fn multi_worker_02_split_spmd_shape_and_tag_discipline() {
    ...
    let scratch = repo_root().join("nucleus/target/mpi-nonblocking-test-scratch/multi_worker_02");
    let _ = std::fs::remove_dir_all(&scratch);

    let res = emit(&r.per_worker, &r.names, &r.sidecar, &kernels, &scratch)
        .expect("mpi-nonblocking multi-worker emit (02-split/split)");
    ...
    // SPMD dispatch + MPI lifecycle + NON-BLOCKING BUFFERED rendezvous
    // prelude + buffered-send buffer attach + barrier.
    for needle in [
        "mpi::initialize()",
        ...
        "if size != 2 {",
        "struct VecChan",
        // Buffered immediate send (MPI_Ibsend) — NOT blocking send_with_tag.
        "immediate_buffered_send_with_tag",
        // Non-blocking matched receive (MPI_Imrecv) — NOT receive_vec_with_tag.
        "immediate_matched_receive_into",
        // MPI_Wait on each request (explicit completion; AC#2).
        "wait_without_status",
        // MPI_Buffer_attach: the buffered-send buffer (deadlock fix).
        "set_buffer_size",
        "NUC_MPI_BSEND_BYTES",
        "struct WorldBar",
        ".barrier();",
...
fn one_worker_push_and_wait_of_same_pair_is_rejected_loud() {
    ...
    let kernels = repo_root().join("nuc-nucleus/examples/02-split-add/kernels.rs");
    let scratch = repo_root().join("nucleus/target/mpi-nonblocking-test-scratch/push_and_wait_same");
    let _ = std::fs::remove_dir_all(&scratch);
