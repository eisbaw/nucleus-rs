...
//! `EmitError` variants on the mpi-blocking backend.
//!
...
//! mpi-blocking so a future refactor that re-introduces an
...
//!
//! NOTE: the mpi-blocking crate is a NORMAL host std crate — `just ci` /
...
//!
//! mpi-blocking layout: the first generated write (the `WriteFailed`
...
use mpi_blocking::{emit, EmitError, NameTables};
...
fn scratch_dir(name: &str) -> PathBuf {
    let target = repo_root().join("nucleus/target/mpi-blocking-test-scratch");
...
/// `WriteFailed`: pre-creating `out_dir/src/kernels.rs` (mpi-blocking's
        ...
        EmitError::WriteFailed { path, .. } => {
            assert_eq!(
                ...
                "WriteFailed must name the src/kernels.rs path it could not write \
                 (mpi-blocking's first generated write)"
            );
        }
