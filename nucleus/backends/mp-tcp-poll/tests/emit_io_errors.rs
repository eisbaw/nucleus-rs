...
//! `EmitError` variants on the mp-tcp-poll backend.
//!
...
//! mp-tcp-poll so a future refactor that re-introduces an
...
//!
//! mp-tcp-poll layout difference vs pthreads-sync: the first generated
...
use mp_tcp_poll::{emit, EmitError, NameTables};
...
fn scratch_dir(name: &str) -> PathBuf {
    let target = repo_root().join("nucleus/target/mp-tcp-poll-test-scratch");
...
/// `WriteFailed`: pre-creating `out_dir/src/kernels.rs` (mp-tcp-poll's
        ...
        EmitError::WriteFailed { path, .. } => {
            assert_eq!(
                ...
                "WriteFailed must name the src/kernels.rs path it could not write \
                 (mp-tcp-poll's first generated write)"
            );
        }
