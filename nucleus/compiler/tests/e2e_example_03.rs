//! End-to-end test for example 03-reduction, pthreads-sync backend
//! (TASK-0022).
//!
...
//!
//! The distributed schedule is shipped as a stretch (per the task
//! brief): the test is `#[ignore]`'d below until the backend learns
//! distributed placement + per-tile transfers (TASK-0117, TASK-0126).
...
fn example_dir() -> PathBuf {
    repo_root().join("nuc-nucleus/examples/03-reduction")
}
...
/// diff) for a given schedule of example 03. Factored so the
/// `naive` and the (currently ignored) `distributed` tests share
/// machinery.
fn run_example_03(sched_rel: &str, scratch_name: &str) {
    let ex = example_dir();
...
fn naive_pthreads_sync_bit_identical() {
    run_example_03(
        ...
        "example_03_naive_pthreads_sync",
    );
}

/// Distributed schedule e2e: currently blocked on TASK-0117
/// (iteration-space partitioning for distributed placement) and
/// TASK-0126 (per-tile transfer codegen). The pthreads-sync backend's
/// `distributed_placement_is_rejected` test pins the rejection
/// at the multi_worker codegen path; until that lifts, this test
/// stays ignored. The schedule itself parses, lowers, and links —
/// the gate is purely on emit/run.
#[test]
#[ignore = "TODO TASK-0117 + TASK-0126: distributed placement + per-tile transfer codegen not yet implemented"]
fn distributed_pthreads_sync_bit_identical() {
    run_example_03(
        "schedules/distributed.sched.nuc",
        "example_03_distributed_pthreads_sync",
    );
}
