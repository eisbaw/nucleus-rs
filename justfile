...
# Production case study (TASK-0455.03): the VGA frame-strip (15360x640,
# ~39MB/array — 32 stacked VGA frames) single-pass 3x3 box-blur stencil
# carried end-to-end at REALISTIC size. Builds + runs naive/pthreads-async
# (single-worker baseline), distributed/pthreads-async (tier-1, 4 workers)
# and distributed/mpi-nonblocking (tier-2, mpiexec -n 5), byte-diffs each
# against an INDEPENDENT reference oracle, and prints the case-study
# numbers (gate cost, runtime vs startup floor, wire-transfer volume).
# Enters the `.#mpi` shell (which carries BOTH the tier-1 cargo toolchain
# and mpiexec). Fixtures (input/reference, ~39MB each) are generated by
# the runner, not committed. Lives under docs/, NOT nuc-nucleus/examples/,
# so the e2e matrix never enumerates it.
# Run the production case study end-to-end (docs/case-study.md).
case-study:
