...
Multi-error AlgoIR lowering: lower_algo now accumulates every genuinely-independent LowerError in one pass (LowerErrors owner — NOT reusing ParseErrors) instead of ?-bailing on the first, and at depth=1 does NOT emit cascade errors (direct references to a declaration that itself failed). DEPTH>1 IS NOT YET TRANSITIVE — a known overcount defect (see ORCHESTRATOR re-gate in notes; the one-line fix at lower.rs case-1 record_decl_failure is queued for a fresh session).
...
Independent-vs-cascade discipline (AC#2 — depth=1):
The symbol table IS the cascade boundary. A declaration that fails to evaluate is not inserted into ir.consts / ir.data / ir.kernels and its name is recorded in Accum::failed_decls (BTreeMap — no hash iteration on the error path; PRD §10.1). Reference errors (UnknownIdent, AssignmentTargetNotData, ConstRefersToNonConst, ShapeRefersToNonConst) whose referenced identifier is poisoned are pure cascade of the already-reported root and are suppressed. Other error kinds are independent properties and are recorded. Duplicate-decl errors do NOT poison the name (the first decl is valid, suppressing here would be undercount — the inverse defect). DEPTH-1 ONLY (CORRECTED 2026-05-20 from the disavowed "applied transitively" rationale): a decl that fails ONLY because its referenced upstream is poisoned does NOT itself get poisoned, so downstream references emit UnknownIdent that the cascade-suppression rule does not match (the name was never declared as far as failed_decls is concerned) — this is the known transitive overcount defect. See lower.rs:195-218 for the code-level honest characterization (commit dcdc302).

Measured counts (AC#3 disclosure at depth=1; AC#3 UNTICKED pending depth>1 fixture):
...
- 1 failed const with N depth=1 dependents -> EXACTLY 1 error (no N-cascade). Parametrised fixture N in {1,2,5,8}; driver-binary cross-check N in {1,2,5} -> 1, 1, 1.
- Combined (the depth=1 discriminating case): 1 root + N suppressed depth=1 dependents + M independents -> EXACTLY 1+M. (N,M) in {1,2,5} x {1,2,3}. Catches both undercount (suppressing independents) and overcount (emitting depth=1 cascade).
- DEPTH>1: PROBE 5 (poisoned const + dependent data + 2 statements using the data) measures 3 errors, expected 1; PROBE 14 (poisoned const + dependent data + for-loop using data) measures 2, expected 1. Known defect, one-line fix queued.
...
Doc honesty (comment-doc-lie class, AC#4): lower.rs module-doc + lower_algo fn-doc rewritten to multi-error reality — no stale aborts-on-first residue. decision-0003 invariant-vs-typed-error split preserved (no new panics on user-reachable paths). The case-1 record_decl_failure docstring at lower.rs:189-218 was subsequently corrected by dcdc302 to disclose the depth=1-only limitation. This tracker entry (DESIGN bullet, FORWARD-CARRY, and Final Summary) was correspondingly rewritten on 2026-05-20 to remove the disavowed "applied transitively at decl level" sentences.
...
- New parametrised cascade fixtures at depth=1 (3 functions, iterating M and N) plus a valid-program-still-lowers guard.
- DEPTH>1 PARAMETRISED FIXTURE STILL TO LAND — this is part of the queued transitive fix; without it AC#3 cannot honestly tick.
...
- cargo test --workspace: 447 passed / 0 failed / 2 ignored (post-TASK-0089).
- cargo clippy --workspace --all-targets -- -D warnings: clean.
- just ci: exit 0.

Scope: AlgoIR lowering only. Sched-lowering multi-error is the separate TASK-0200 (same independent-vs-cascade problem, different layer — the design here is the template MINUS the disavowed transitive claim; sched-lowering should pick up the corrected design once the one-line fix here lands). Commit 0548f02 (multi-error infra) + dcdc302 (code-level docstring honesty correction). Status: In Progress; the multi-error feature is real and working at depth=1, the transitive overcount is a precisely localized known defect with the one-line fix specified for a fresh session.
