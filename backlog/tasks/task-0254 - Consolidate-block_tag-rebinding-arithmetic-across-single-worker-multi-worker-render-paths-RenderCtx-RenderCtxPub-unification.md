...
- [ ] #1 Decision recorded (a/b/c).
- [ ] #2 The 4 format-string sites in the codebase collapse to ONE site (or to a shared sub-helper called by 2 sites that cannot drift in arithmetic).
- [ ] #3 All blocked single-worker e2e cells (04/blocked, 05/blocked, 06/blocked, 07/blocked) stay byte-identical-green; multi-worker blocked tests stay green.
- [ ] #4 `just determinism-check` clean; `just e2e` 88/70/0/18 preserved.
- [ ] #5 The TASK-0253 helper docstring's carve-out note ("a separate sibling copy survives on the single-worker render path") + the mp-tcp-bufsync call-site comment can be deleted as obsolete.
