...
- qa-test-runner: GO. Critical regression re-check: 16-jacobi/distributed builds FAST now (e2e cells PASS 0.77-4.58s, NOT the prior ~15-min hang); direct driver pre-compile <1s. build OK; clippy clean; test 1254 dev / 1252 release; e2e 385/328/0/57/0 (~6.6min wall, no hang); perf-pin gate_stays_near_linear 0.40s; all 3 new tests pass.
- mped-architect: NO-GO with 2xP1 + P2 + P3 — ALL folded back in commit eaa7271, then re-verified GO:
  * P1 (tautology): off_order prove-it-bites test had transitions [load1,load2,cons_x,...] so derive_firing_order reached p==2 ON the derived order -> OLD code would have caught it too; test did not exercise the completeness gain. FIXED: reordered to [load1,cons_x,load2,cons_y,cons_z] (architect original) + added empirical max_p==1 assertion proving the derived order never reaches p==2 (so old replay accepted). Now genuinely bites.
  * P1 (doc-lie): check_net_sound comment still claimed "short-circuits on every shipping net before BFS/fallback" (the false premise). FIXED + grep-swept (only instance).
  * P2: filed TASK-0427.01 (65..N-transition completeness gap) + referenced from CONFLICT_BFS_TRANSITION_LIMIT docstring.
  * P3: softened "BFS stays fast" -> "bounded by marking cap".
  * Architect VERIFIED correct (no action): soundness (find_conflict_at_marking/is_consumption_enabled byte-identical old vs new, BFS over strict superset of markings -> cannot false-reject); determinism (FIFO + TransitionId-order successors + canonical marking key, absent==zero dedup); cap/NUC_TRACE advisory honest; no silent-sibling / dead code; fallback body is prior logic preserved.

MAJOR PROCESS NOTE: the orchestrator design premise "no shipping net has a contested place -> BFS never runs on shipping nets" was FALSE (16-jacobi/distributed gate net has 24 contested places). An unbounded BFS would have shipped a ~15-min e2e hard regression. The implementer caught it EMPIRICALLY (built the net, probed contested-place count via NUC_TRACE, reproduced the deterministic spin) rather than trusting the orchestrator narrative. Root-fixed with CONFLICT_BFS_TRANSITION_LIMIT=64 size guard (fallback to sound single-order replay above it). Prior-session uncommitted work had misdiagnosed the same spin as a "transient build flake".

TASK-0427 stays DONE: BFS coverability completeness for small contested nets, sound fallback for large, gate green, both P1s fixed, independent review GO.
<!-- SECTION:NOTES:END -->
