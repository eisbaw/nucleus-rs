...
<!-- SECTION:NOTES:BEGIN -->
ORCHESTRATOR REVIEW GATE (2026-05-31, independent, read-only): GO x2. qa-test-runner RE-RAN the gate (not transcribed from implementer): build clean; clippy exit 0 (forced non-cached re-lint, no doc_lazy_continuation); test dev 1157/0/3 (render_gather_negative = 5 passed); test-release 1156/0/3 (the -1 is the known TASK-0291 debug_assert #[should_panic] divergence, not a regression); e2e 329/272/0/57/0 on two byte-identical runs. mped-architect traced all 5 tests to their intended guards (each passes the earlier checks to reach its arm; positive-control snapshot col[(k) as usize] confirmed via render_flat_index rank-1 early-return) and found no mis-target. P3.1 (other render fail-loud siblings untested) filed as TASK-0379.
<!-- SECTION:NOTES:END -->
