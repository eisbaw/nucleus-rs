...
\begin{figure}[tbp]
  ...
  \begin{tikzpicture}[
    ...
    \foreach \row/\lab [count=\ri from 0] in {%
        ...
        {P,P,P,P,P,P,P}/19, {P,P,P,P,P,P,P}/20, {P,P,P,P,P,P,P}/21,
    ...
    % legend --- anchored north-west so multi-line entries grow downward from a
    % fixed top edge and cannot overlap their neighbours.
    \node[cell, P, anchor=north west] at (9.0, -1.5) {};
    \node[anchor=north west, font=\scriptsize, align=left, text width=37mm]
      at (9.7,-1.4) {all required cells pass};
    \node[cell, M, anchor=north west] at (9.0, -3.1) {};
    \node[anchor=north west, font=\scriptsize, align=left, text width=37mm]
      at (9.7,-3.0) {mixed: some schedule cells skipped (capability mismatch, or
       an embedded-tier example validated by the Renode gate instead)};
    \node[cell, s, anchor=north west] at (9.0, -7.4) {};
    \node[anchor=north west, font=\scriptsize, align=left, text width=37mm]
      at (9.7,-7.3) {all skipped (carried by a capable sibling, or honestly
       unsupported)};
    \node[anchor=north west, font=\scriptsize\itshape, align=left, text width=37mm]
      at (9.0,-10.2) {rows are the 26 examples in the tier-1 matrix
