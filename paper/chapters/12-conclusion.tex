...
This document began with a narrow, concrete complaint: that porting a parallel
...
twenty-nine worked examples through ten
...
static, single-assignment, and integer-valued at its core --- unordered
floating-point summation is excluded by the determinism it would break, and the
opt-in fixed-order variant is reproducible across backends but not across
schedules; there is no data-dependent control flow,
...
oversights, and the document has tried throughout to weigh them with the same
...
is the one the document stands behind.
