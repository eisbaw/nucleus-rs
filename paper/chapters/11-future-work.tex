...
test preserved). On this single-worker path the construct is no longer one-backend: both
examples now run byte-identically across all seven tier-1 backends, because a host-only
schedule delegates to the shared single-worker emitter on every backend
(\cref{sec:res-quant}). Two residuals remain. First, the break is single-worker only: on the
multi-worker (distributed, \texttt{partition=workers}) path the convergence scalar is a
cross-worker reduction, so the early
...
pattern not yet built. This distributed form is named future work rather than recorded as
a skipped cell: it cannot become an executable cell until the distributed Jacobi base it
builds on lands, so the multi-worker break-condition path stays fail-loud-rejected in the
meantime rather than silently passed.
